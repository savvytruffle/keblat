...
class Keblat(object):
    ...
    def getpars(self, partype='allpars'):
        ...
        if partype == 'allpars':
            ...
            return np.array([self.pars['msum'], self.pars['mrat'], self.pars['z0'], self.pars['age'],
        ...
        for ii in range(2):       
            ...
            for jj in range(2):
                ...
                if (m0<=maxm) & (m0>=minm):
                    ...
                    #sorted_minds = np.argsort(self.iso[goodindsz[0]:goodindsz[1], self.isodict['mact']][goodindst[0]:goodindst[1]])
                    sorted_minds = np.arange(0, len(self.iso[goodindsz[0]:goodindsz[1], self.isodict['mact']][goodindst[0]:goodindst[1]]))
                    goodindsm0 = np.searchsorted(self.iso[goodindsz[0]:goodindsz[1], self.isodict['mact']][goodindst[0]:goodindst[1]], mvals[mbounds], sorter=sorted_minds)
                    goodindsm1 = np.searchsorted(self.iso[goodindsz[0]:goodindsz[1], self.isodict['mact']][goodindst[0]:goodindst[1]], mvals[mbounds], sorter=sorted_minds, side='right')
                    try:
#                        intergrid[ii, jj] = interpolate.interp1d(self.iso[goodindsz[0]:goodindsz[1], self.isodict['mact']][goodindst[0]:goodindst[1]][goodindsm0[0]:goodindsm1[1]], self.iso[goodindsz[0]:goodindsz[1]][goodindst[0]:goodindst[1]][goodindsm0[0]:goodindsm1[1]][:, self.ipinds], axis=0, bounds_error=False)
                        intergrid[ii, jj, :] = np.array([np.interp(m0, self.iso[goodindsz[0]:goodindsz[1], self.isodict['mact']][goodindst[0]:goodindst[1]][sorted_minds][goodindsm0[0]:goodindsm1[1]], zz) for zz in self.iso[goodindsz[0]:goodindsz[1]][goodindst[0]:goodindst[1]][sorted_minds][goodindsm0[0]:goodindsm1[1]][:, self.ipinds].T])
...
#        lc_uncertainty = np.array([0.002, 0.002, 0.002])
        if np.any(np.isinf(lc_inputs)):
            if residual:
                return np.ones(len(self.magsobs) + len(lc_priors))*1e12
            else:
...
#                            (-8, 1.)])
        bounds = np.array([self.parbounds['msum'], self.parbounds['mrat'], 
                           ...
                           self.parbounds['dist'], 
                           self.parbounds['ebv'], self.parbounds['h0'], 
                           self.parbounds['isoerr']])
...
#                totmod[:, ii][se][bad] = np.nan
    ...
    def plot_lcrv(self, allpars, prefix, suffix='lcrv', savefig=True, 
                  polyorder=2, ooe=True, thresh=10.):
